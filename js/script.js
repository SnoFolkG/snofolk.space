...
function findSimilarAlbums(currentAlbum, allAlbums) {
  ...
  const sameArtist = shuffleArray(
    others.filter((a) => a.artist === currentAlbum.artist),
  );

  const sameCity = shuffleArray(
    others.filter(
      (a) => a.city === currentAlbum.city && a.artist !== currentAlbum.artist,
    ),
  );
...
function renderAlbumDetail(albums) {
  ...
  const similarCardsHTML = displayAlbums
    .map(
      (a) => `
...
function sortDownloadsAlbums(albums, sort = downloadsState.sort) {
  ...
  return albums.slice().sort((a, b) => {
    const artistCompare =
      (a.artist || "").localeCompare(b.artist || "") * direction;
